...
`サブルーチン`とは、一連の処理をまとめたプログラム。またの名を`関数`。`sub`命令で定義できる。
...
サブルーチン名の後ろに丸括弧をつけてやると実行。

    make_ramen();
    
これを`サブルーチンのコール`とか`関数呼び出し`などと言う。
...
実際にコールするときには、以下のようにする。

    eat('sushi', 'chopsticks');
    # とか
    eat('steak', 'knife and fork');

この時、`'sushi'`や`'shopsticks'`のことを`引数`と呼び、サブルーチンのコール時に引数を指定することを`引数を渡す`と表現する。

eatサブルーチンを定義する時に利用された`@_`は、**引数が格納された特殊な配列**である。**とにかく`@_`は覚えよう。**`@_`です。
