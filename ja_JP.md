...
* printという命令を使うと、ターミナルに任意の文字列を表示できる
* "（二重引用符/ダブルクォート）を使うと、文字列を表現できる。
...
* 三角形の面積 = (幅 x 高さ)÷2 を、$triangleを求める式で表現。
* printf命令は、最初の引数でフォーマットを指定し、%dとか%sとか%0.03fみたいな場所に、続きの引数を順番に当て込んで、ターミナルに出力。
...
配列の中身が一つ一つ、$itemに代入され、ループ内で処理するのに使えるようになる。
...
$conditionには、何らかの判定結果を入れる。
すると、$conditionが0かnullにならない限りは``無限``にループする。
